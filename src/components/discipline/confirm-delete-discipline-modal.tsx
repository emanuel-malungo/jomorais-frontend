...
interface ConfirmDeleteDisciplineModalProps {
  open: boolean;
...
export function ConfirmDeleteDisciplineModal({
  open,
  onOpenChange,
  onConfirm,
  discipline,
  ...
  error,
  deleteResult,
}: ConfirmDeleteDisciplineModalProps) {
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center space-x-2 text-green-600">
              ...
              <span>Disciplina Excluída com Sucesso!</span>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <Alert className="border-green-200 bg-green-50">
              ...
              <AlertDescription className="text-green-800">
                {deleteResult.info || 'A disciplina foi excluída com sucesso.'}
              </AlertDescription>
            </Alert>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2 text-red-600">
            ...
            <span>Confirmar Exclusão de Disciplina</span>
          </DialogTitle>
          <DialogDescription>
            Esta ação não pode ser desfeita. A disciplina será excluída permanentemente.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between p-3 bg-gray-50 rounded-lg">
              <span className="text-sm font-medium text-gray-600">Nome da Disciplina:</span>
              <span className="text-sm font-semibold text-gray-900">{discipline.designacao}</span>
            </div>
            <div className="flex justify-between p-3 bg-gray-50 rounded-lg">
              <span className="text-sm font-medium text-gray-600">Curso:</span>
              <span className="text-sm font-semibold text-gray-900">
                {discipline.tb_cursos?.designacao || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between p-3 bg-gray-50 rounded-lg">
              <span className="text-sm font-medium text-gray-600">Status:</span>
              <span className={`text-sm font-semibold ${discipline.status === 1 ? 'text-green-600' : 'text-gray-600'}`}>
          ...
          <Alert className="border-yellow-200 bg-yellow-50">
            ...
            <AlertDescription className="text-yellow-800">
              <strong>Exclusão em Cascata!</strong> Ao excluir esta disciplina, todos os dados associados serão removidos permanentemente,
              incluindo itens na grade curricular.
            </AlertDescription>
          </Alert>
          ...
          <Alert className="border-red-200 bg-red-50">
            ...
            <AlertDescription className="text-red-800">
              <strong>Atenção!</strong> Esta ação não pode ser desfeita e removerá permanentemente a disciplina e todos os seus dados relacionados do sistema.
            </AlertDescription>
          </Alert>
