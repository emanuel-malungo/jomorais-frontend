...
export interface ISAFTFile {
  header: ISAFTHeader;
  masterFiles: {
    generalLedgerAccounts?: any[]; // Contas do razão geral
    customers?: ISAFTCustomer[];
    suppliers?: any[]; // Fornecedores
    products?: ISAFTProduct[];
    taxTable?: ISAFTTaxTable[];
  };
  generalLedgerEntries?: any[]; // Movimentos contabilísticos
  sourceDocuments: {
    salesInvoices?: {
    ...
    movementOfGoods?: any; // Movimentos de mercadorias
    workingDocuments?: any; // Documentos de trabalho
    payments?: {
      numberOfEntries: number;
      totalDebit: number;
      totalCredit: number;
